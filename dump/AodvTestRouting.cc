...
void AodvTestRouting::receivePktRREQ(PacketRREQ* pkt,int srcMacAddress, double rssi, double lqi)
{
    ...
    int pDelay = pkt->getpropDelay();//added the following statements Raj 
    pDelay = (int)clock() - pDelay;//added the following statements Raj
    ...
    if(string(pkt->getDstIP()).compare(SELF_NETWORK_ADDRESS)==0) //current node is the destination (RFC3561 chapter 6.6.1)
    {
	    ...
	    trace() << "AODV : RREQ : reach destination - origin: " << string(pkt->getSrcIP())
                                                        << " id: " << pkt->getRreqID();
	    trace() << "AODV : RREP : succ : sent (is the destination)";//added the pdelay here Raj
	...
	sendPktRREQ(hopcount, id, srcIP, dstIP, srcSN, dstSN,(int)clock());//added parameter pDelay by Raj on 19/10/18
...
void AodvTestRouting::sendPktRREQ(int hopCount, int id, string srcIP, string dstIP, unsigned long srcSN, unsigned long dstSN, int pDelay)//added pdelay by Raj
{
